...
object SVDPlusPlusApp {
  ...
  if(dataset=="small"){
   ...
   edges.persist()
      ...
      edges = new ProvRDD(sc.textFile(input)).map { line =>
    ...
    var err = new ProvRDD(newgraph.vertices).collect().map{ case (vid, vd) =>

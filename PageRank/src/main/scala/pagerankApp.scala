...
object pagerankApp {
  ...
  def main(args: Array[String]) {
    ...
    val staticRanks = new ProvRDD(graph.staticPageRank(maxIterations, resetProb).vertices)
  ...
  def pagerank_usingSampledata(sc: SparkContext, input: String, output: String,
    ...
    val ranks = new ProvRDD(graph.pageRank(tolerance, resetProb).vertices)

    // Join the ranks with the usernames
    val users = new ProvRDD(sc.textFile(input + "/users.txt")).map { line =>

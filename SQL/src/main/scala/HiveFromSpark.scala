...
object HiveFromSpark {
  ...
  if(storageLevel=="MEMORY_AND_DISK_SER")
	...
	val orderRdd = new ProvRDD(sc.textFile(inputPath+"/OS_ORDER.txt",numpar)).map{line =>
		val data=line.split("\\|")
	...
	val oitemRdd = new ProvRDD(sc.textFile(inputPath+"/OS_ORDER_ITEM.txt",numpar)).map{line =>
		val data=line.split("\\|")
